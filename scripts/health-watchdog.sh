...
private_base="${PRIVATE_SERVICE_URL:-${HERMES_PRIVATE_BASE_URL:?HERMES_PRIVATE_BASE_URL is required}}"
token_file="${HERMES_MAINTAINER_TOKEN_FILE:-${HERMES_ADMIN_TOKEN_FILE:?HERMES_ADMIN_TOKEN_FILE is required}}"
backup_dir="${HERMES_BACKUP_DIR:-/backups}"
cert_file="${HERMES_CERT_FILE:-/etc/nginx/ssl/tailscale/hermes.crt}"
max_age="${HERMES_MAX_AGE_SECONDS:-${HERMES_BACKUP_MAX_AGE_SECONDS:-172800}}"
...
status() {
    curl --silent --insecure --show-error --output /dev/null --write-out '%{http_code}' \
...
if [ -d "$backup_dir" ]; then
    ...
    [ "$(docker exec cti-hermes-backup-1 test -s /backups/latest.metadata 2>/dev/null && echo ok || true)" = ok ] || add_failure "backup missing"
fi
